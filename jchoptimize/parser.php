...
URLREGEX;
        ...
        public function getBodyHtml()
        {
                ...
                {
                        $this->sBodyHtml = parent::getBodyHtml();
                }
        ...
        protected function isHttpAdapterAvailable($sPath)
        {
                ...
                {
                        return (!$this->oFileRetriever->isHttpAdapterAvailable() && ((preg_match('#^http#', $sPath))));
                }
                else
                {
                        return parent::isHttpAdapterAvailable($sPath);
                }
        }
        ...
        public function runCookieLessDomain()
        {
                ...
                {
                        JCH_DEBUG ? JchPlatformProfiler::mark('beforeRunCookieLessDomain plgSystem (JCH Optimize)') : null;
                        ...
                        $base = preg_quote(substr(JchPlatformUri::base(TRUE), 1));

                        $static_match = '(?(?<=["\'])(?=[^"\']+\.(?>' . $static_files . ')["\')])|(?=[^\'"\s>)]+\.(?:' . $static_files . ')[\s)]))';
                        $match        = $static_match . '(?:(?:(?:https?:)?//' . $host . ')?/(?!/)(?:' . $base . '/)?([^\'")]++)|(?!http|/)([^\'")]++))';

                        $cdn_static = JchOptimizeHelper::cookieLessDomain($this->params) . JchOptimizeHelper::getBaseFolder();

                        $aRegexes = array(
                                '#(?:(?=(?>(?:<(?!style))?[^<]*+)?</style)|(?><?[^<]*+)*?<(?:style|/(?:body|head))[^>]*+>)'
                                . '(?:(?>\(?[^(<>]*+)*?(?<=url)\(["\']?\K' . $match . '|\K$)#iS',
                                '#(?><?[^<]*+)*?(?><(?:link|script|img)(?>=?[^=>]++)*?(?<=href| src)=["\']?\K'
                                . $match . '|\K$)#iS',
                                '#(?><?[^<]*+)*?(?>(?>=?[^=>]++)*?(?<=style)=[^(>]++(?<=url)\(["\']?\K' . $match . '|\K$)#iS'
                        );

                        foreach ($aRegexes as $sRegex)
                        {
                                $sProcessedHeadHtml = preg_replace($sRegex, $cdn_static . '$1$2', $this->getHeadHtml());
                                $sProcessedBodyHtml = preg_replace($sRegex, $cdn_static . '$1$2', $this->getBodyHtml());
                        ...
                        JCH_DEBUG ? JchPlatformProfiler::mark('afterRunCookieLessDomain plgSystem (JCH Optimize)') : null;
                }
        }
